...
const ListAllOrders = () => {
  ...
  //Breadcrumb -----------------------------------------------------------------
  const paths: PathInterface[] = [{ name: "Análisis" }, { name: "Órdenes" }];
      ...
      payload: {
        "Número de orden": (
          <div className="flex flex-col">
            ...
            <p className="text-gray-400 text-sm">{`${item?.billing?.firstName ?? ""
        ...
        "Dirección de envío": (
          <p className="text-justify">{`${item?.shipping?.street_1 ?? ""} ${item?.shipping?.street_2 ?? ""
            } ${item?.shipping?.city ?? ""} ${item?.shipping?.municipality?.name ?? ""
            } ${item?.shipping?.province?.name ?? ""} ${item?.shipping?.country?.name ?? ""
            }`}</p>
        ),
        "": (
          <div className="inline-flex gap-2 ">
            {!item.pickUpInStore && (
              <FaTruckFast className="text-xl text-gray-800" />
            )}
            {!!item?.shippingById && (
              <FaPersonWalkingLuggage className="text-xl text-gray-800" />
            )}
            {!!item?.paidAt && (
              <FaMoneyBillWave className="text-xl text-gray-800" />
            )}
          </div>
        ),
      },
  ...
  const allOrdersOrigin: SelectInterface[] = [
    {
      name: "Online",
      id: "online",
    },
