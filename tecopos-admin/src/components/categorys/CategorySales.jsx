...
import { updateSalesCategories } from "../../store/nomenclatorSlice";
...
export default function CategorySales(props) {
    ...
    const onSubmit = async () => {
        ...
        let categoryTemp = { ...values, isActive };
        ...
        if (category) {
            APIServer.patch(
                `/administration/salescategory/${category.id}`,
                categoryTemp
            )
                .then(() => {
                    setLoading(false);
                    toast.success("La categoría se actualizo correctamente");
                    onClose();
                    setReload(!reload);
                })
                .catch(() => {
            ...
            delete categoryTemp.index;

            APIServer.post(`/administration/salescategory`, categoryTemp)
                .then(() => {
                    setLoading(false);
                    onClose();
                    toast.success("La categoría se creo correctamente");
                    setReload(!reload);
                })
                .catch(() => {
                    setLoading(false);
    ...
    const deleteCategory = () => {
        setLoading(true);
        APIServer.deleteAPI(`/administration/salescategory/${category.id}`)
            .then(() => {
                setLoading(false);
                setReload(!reload);
                onClose();
