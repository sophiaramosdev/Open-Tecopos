        ...
        {imgPreview.map((item, idx) => (
          <div key={idx} className="relative">
            <ImageComponent
              ...
              hash={item.hash}
            />
            <TrashIcon className=" absolute top-2 right-2 h-8 p-1 text-red-500 border border-red-500 rounded-md bg-red-100 cursor-pointer active:shadow-md active:shadow-red-700" onClick={()=>deleteImg(idx)}/>
          </div>
        ))}
      </div>
    </div>
