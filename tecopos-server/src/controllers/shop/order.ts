...
//Deprecated
export const newOnlineOrder = async (req: any, res: Response) => {
  ...
  try {
    ...
    const enable_delivery =
      configurations.find((item) => item.key === "enable_delivery")?.value ===
      "true";
    ...
    let orderTemplate: any = {
      ...
      managedById: user.id,
      customerNote,
      origin: order_origin === "Tecopos-ShopApk" ? "shopapk" : "shop",
      clientId: client?.id,
    ...
    // Obtaining order
    const order_to_emit = await OrderReceipt.scope("public_return").findByPk(
      order.id,
      {
        transaction: t,
      }
    ...
    if (order_to_emit.shipping?.email) {
      ...
      if (
        ...
        order_to_emit.shipping?.email !== order_to_emit.billing?.email
      ) {
        emailQueue.add(
          {
            code: "NEW_ORDER_NOTIFICATION",
...
export const newOnlineOrderV2 = async (req: any, res: Response) => {
  const t = await db.transaction(config_transactions);
  const definedCodeCurrency = req.header("X-Shop-CodeCurrency");
  const order_origin: app_origin = req.header("X-App-Origin");
  ...
  try {
    ...
    if (shippingPrice) {
      if (shippingPrice.codeCurrency === definedCodeCurrency) {
        ...
        const convertedPrice = exchangeCurrency(
          shippingPrice,
          definedCodeCurrency ?? main_currency.currency.code,
          availableCurrencies
        );
    ...
    for (const selledProduct of productsToSell) {
      ...
      if (["MENU", "ADDON", "SERVICE", "COMBO"].includes(productDetails.type)) {
    ...
    //Send mail
    let listToNotify: Array<{ email: string; isOwner: boolean, isBuyer?:boolean }> = [];
...
export const findMyOrders = async (req: any, res: Response) => {
  try {
    ...
    const searchable_fields = ["createdAt", "status"];
      ...
      include: [
        {
          ...
          include:[{model:Price, as:"priceUnitary", attributes:["amount", "codeCurrency"]}],
          separate: true,
        },
        {
          model: Price,
...
export const findMyDeliveryOrders = async (req: any, res: Response) => {
  try {
    const { per_page, page, order, orderBy, dateFrom, dateTo, ...params } =
      req.query;
    const user: User = req.user;
    ...
    //Preparing search
    let where_clause: any = {};
    const searchable_fields = ["createdAt", "status", "operationNumber"];
    const keys = Object.keys(params);
    keys.forEach((att) => {
      if (searchable_fields.includes(att)) {
        where_clause[att] = params[att];
...
export const cancelMyOrder = async (req: any, res: Response) => {
  ...
  try {
    ...
    if (["BILLED", "CANCELLED", "REFUNDED"].includes(order.status)) {
      t.rollback();
      return res.status(400).json({
    ...
    if (!order.isPreReceipt) {
      ...
      result = await restoreProductStockDisponibility(
        {
    ...
    if (!internalCheckerResponse(cancelResult)) {
      ...
      Logger.error(cancelResult.message || "Ha ocurrido un error inesperado.", {
        origin: "shop/cancelMyOrder/afterOrderCancelled",
...
export const editOrder = async (req: any, res: Response) => {
  ...
  try {
    ...
    if (
      ["CANCELLED", "REFUNDED", "DELIVERED", "BILLED"].includes(order.status)
    ) {
      t.rollback();
      return res.status(400).json({
...
export const getOrder = async (req: any, res: Response) => {
  try {
    ...
    if (order?.clientId !== client.id && order.shippingById !== user.id) {
      return res.status(401).json({
        message: `No tiene acceso al recurso solicitado.`,
      });
    }

    const to_return = await OrderReceipt.scope("public_return").findByPk(id);
