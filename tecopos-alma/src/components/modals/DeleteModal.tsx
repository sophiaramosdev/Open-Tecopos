...
export default function DeleteModal({state,switchState,item,action, isFetching}:DeleteModalProps) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div className="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                  <button
                    ...
                    onClick={() => switchState && switchState(false)}
                  >
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Confirma que desea eliminar <span className='font-bold'>{item}</span>?
                      </p>
                    </div>
                  </div>
                </div>
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    disabled={isFetching}
                  >
                    {isFetching ? <span className='animate-pulse'>Eliminando</span> : <span>Confirmar</span> }
                  </button>
                  <button
                    type="button"
