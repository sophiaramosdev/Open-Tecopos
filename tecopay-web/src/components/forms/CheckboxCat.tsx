import { BasicNomenclator } from '../../interfaces/localInterfaces';
...
export default function Checkbox({
	...
	const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (e.target.checked) {
			//console.log(data.filter((item) => item.id === currentValue)[0].id)
			setSelected(data.filter((item) => item.id === currentValue));
		}
	};
	...
	return (
		<fieldset>
			<div
				className={`${displayCol ? 'flex justify-center' : 'grid grid-cols-4 gap-5'}`}
			>
				{data.map((item, idx) => (
