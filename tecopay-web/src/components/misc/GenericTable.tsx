...
import { FilterCodeDatePickerRange } from './SearchCriteriaComponent';
import {
	BasicNomenclator,
	BasicType,
	SelectInterface,
} from '../../interfaces/localInterfaces';
...
const GenericTable = ({
	...
	//Manage Filter --------------------------------------------------------------------------------------
	const { control, handleSubmit, unregister, register, watch, reset } =
		useForm();
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 items-center w-full justify-center'>
							{activatedFilters?.map((item, idx) =>
								item.format === 'select' && item.asyncData ? (
									...
									>
										<input
											className='h-4 w-4 rounded border-indigo-600 focus:bg-transparent text-slate-400 focus:ring-transparent'
