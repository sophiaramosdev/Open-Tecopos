...
const initialState = (
	...
	return filters
		.filter((filter) =>
			isMultipleFilterBtnActive ? filter.isRequired : filter,
		)
		.map((filter) => ({
			id: filter.name,
...
const SearchCriteriaComponent = ({
	...
	const [selectedFilters, setSelectedFilters] = useState<Record<string, any>[]>(
		initialState(
			filters,
			isMultipleFilterBtnActive !== undefined
				? isMultipleFilterBtnActive
				: true,
		),
	);
		...
		switch (filter.type) {
			...
			case 'boolean':
				return (
					<label
						key={filter.name}
						className={`flex gap-2 text-sm font-medium items-center ${
							errors[filter.name] ? 'text-red-900' : 'text-gray-700'
						}`}
					>
						<input
		...
		>
			<div className='grid grid-cols-3 gap-4'>
				{selectedFilters.length > 0 &&
					selectedFilters.map((filter) => generateFilterComponent(filter))}
			</div>
			<div
				className={`flex ${
					selectedFilters.length ? 'pt-5 justify-end' : 'justify-between'
				}`}
			>
				{selectedFilters.length === 0 && (
				...
				<div className='flex gap-4 justify-end'>
					{(isMultipleFilterBtnActive ||
						...
						<MultipleFilterBtn
							...
							setSelected={(data: BasicType[], unregistered?: string) => {
