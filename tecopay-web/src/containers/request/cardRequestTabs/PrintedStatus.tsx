...
const PrintedStatus = () => {
	...
	const filterAction = (data: any) => {
		data
			? setFilter({ ...data, status: 'PRINTED' })
			: setFilter({ page: 1, status: 'PRINTED' });
	};
...
const ModalCardRequest = ({ CRUD, id, close }: propsDestructured) => {
	...
	requestRecord.forEach((item: any) => {
		switch (item.status) {
			...
			case 'CLOSED':
				body['Impresa el'] = `${moment(item.createdAt).format('DD-MM-YYYY')}`;
				break;
	...
	return (
		<>
			<div className='flex'>
				<div>
					<p className='mb-4 font-semibold text-lg text-green-600'>IMPRESA</p>
